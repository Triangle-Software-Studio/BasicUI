...
### 2025-07-13 — 漏洞修复与代码加固
- **MenuBar**: 修复 `MouseMove` 中 `activeMenu_` 超出菜单数量时的越界访问风险。空菜单现在正确返回零尺寸下拉框。
- **文本输入**: `Window::PollEvent` 现在将多字符 `SDL_TEXTINPUT` 事件加入队列，不再只解码第一个 UTF-8 码点后丢弃后续字符。
- **Unicode 截断**: `ListView`、`CheckBox`、`ProgressBar`、`Tooltip`、`Panel` 和 `MenuBar` 已全部改用 UTF-8 感知的 `Utf8CharCount` / `Utf8Substr`，替代按字节截断的 `std::string::size()` / `substr()`，避免中文等多字节文本被截断到字符中间。
- **RadioButton**: 新增同组自动互斥机制。选中一个单选按钮后，会自动取消同组其他按钮的选中状态，并触发它们的 `onToggle(false)` 回调。
- **窗口尺寸**: `ResizeToCells` 增加 `int` 乘法溢出检查，防止超大窗口尺寸导致未定义行为。
- **线程安全注释**: 在 `GlyphAtlas` 中添加文档说明，所有操作必须在持有 OpenGL 上下文的同一线程执行。
...
关于字体的说明：本项目中使用的字体，其版权及使用许可均遵循字体原作者或发布者所声明的协议，与本项目采用的 MIT 许可证无关。
